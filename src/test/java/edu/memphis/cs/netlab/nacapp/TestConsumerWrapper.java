...
public class TestConsumerWrapper {
	...
	static class LocalTestFace extends Face implements Producer.OnEncryptedKeys {
		...
		private void processCKey(Interest interest, OnData onData) {
			if (null != cKeys) {
			...
			Producer.OnEncryptedKeys onEncryptedKeys = new Producer.OnEncryptedKeys() {
				...
				public void onEncryptedKeys(List list) {
					...
					if (cKeys.size() == 0) {
						cKeys = null;
						Global.LOGGER.warning("Producer : Error producing content key: got 0 encrypted keys.");
						return;
					}
					onData.onData(interest, cKeys.get(0));
				}
...
//				d.getMetaInfo().setFreshnessPeriod(1000);
			...
			try {
				...
				// create content
				fixture.producer.produce(d, fixture.startTimeSlot, new Blob(fixture.plainTextData), onError);
			...
			try {
				if (eKeys == null || eKeys.size() == 0) {
					...
					for (int i = 1; i < keys.size(); i++) {
						...
						Global.LOGGER.info(
							String.format("new d-key: %s", key.getName().toUri()));
	...
	static class FixtureStatics {
		...
		public FixtureStatics() {
			try {
				...
				interval = new RepetitiveInterval(
					fromIsoString("20150825T000000"),
					fromIsoString("21010825T000000"),
					0,
					24,
					1,
		...
		public Fixture() {
			...
			try {
				...
				producer = new Producer(
					prefix, datatype, face, keychain, producerDb);
	...
	public void testGenerateKeyPair() {
		try {
			...
			Blob encryptedBlob = RsaAlgorithm.encrypt(
				pair.publicKey.getKeyBits(),
				plaintext,
				encryptParams
			);
		...
		try {
			...
			Consumer.OnConsumeComplete onConsume = new Consumer.OnConsumeComplete() {
				...
				public void onConsumeComplete(Data data, Blob blob) {
					...
					Assert.assertArrayEquals(blob.getImmutableArray(), fixture.plainTextData.getImmutableArray());
				}
			};
			EncryptError.OnError onError = new EncryptError.OnError() {
				@Override
